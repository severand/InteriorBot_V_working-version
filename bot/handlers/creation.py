...
# --- ОБНОВЛЕНО: 2026-01-02 20:30 - ИСПРАВЛЕНО УДАЛЕНИЕ СТАРОГО МЕНЮ ---

# [2026-01-02 20:30] ДОБАВЛЕНО: Явное удаление старого menu_message_id перед отправкой новой картинки
# [2026-01-02 20:30] ИСПРАВЛЕНО: Обновлено в exterior_prompt_received() и room_description_received()
# [2026-01-02 20:30] РЕЗУЛЬТАТ: При загрузке нового фото больше не остаются старые сообщения в чате
...
[2026-01-02 20:30] ИСПРАВЛЕНО: Явное удаление старого menu_message_id перед отправкой фото
...
# [2026-01-02 20:30] ДОБАВЛЕНО: Явно удаляем старое меню перед отправкой фото
...
logger.info(f"🗑️ [EXTERIOR_PROMPT] ✅ Deleted old menu msg_id={old_menu_id}")

except Exception as e_delete:

logger.warning(f"⚠️ [EXTERIOR_PROMPT] Could not delete old msg_id={old_menu_id}: {e_delete}")
...
# Создаём меню с ошибкой

text_with_balance = await add_balance_and_mode_to_text("❌ Ошибка генерации. Попробуйте еще раз.", user_id)

error_menu = await message.answer(
...
await state.update_data(menu_message_id=error_menu.message_id)

await db.save_chat_menu(chat_id, user_id, error_menu.message_id, 'generation_error')
...
logger.info(f"🗑️ [ROOM_DESC] ✅ Deleted old menu msg_id={old_menu_id}")

except Exception as e_delete:

logger.warning(f"⚠️ [ROOM_DESC] Could not delete old msg_id={old_menu_id}: {e_delete}")
...
caption = f"✨ Ваш новый дизайн {room_name} в стиле {style_name}!"
