...
#    • SCREEN 6→2: new_photo_after_gen() - Загрузка нового фото (без генерации)
#
...
async def room_choice_handler(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if current_msg.photo:
            ...
            # Текущое сообщение - ТЕКСТ → редактируем обычно
            await edit_menu(
                callback=callback,
                state=state,
                text=text,
...
# ═════════════════════════════════════════════════════════════════════════════════════════════════════
# 📸 [SCREEN 6→2] ЗАГРУЗКА НОВОГО ФОТО ПОСЛЕ ГЕНЕРАЦИИ (NEW PHOTO AFTER GEN)
# ═════════════════════════════════════════════════════════════════════════════════════════════════════
# 📍 ЭКРАН: 6→2 (навигация на загрузку фото)
# 📊 FSM STATE: CreationStates.post_generation → CreationStates.uploading_photo
# 🎯 НАЗНАЧЕНИЕ: Очистить дизайн и вернуться к загрузке нового фото для новой генерации
# ⬅️ ПРЕДЫДУЩИЙ ЭКРАН: SCREEN 6 (меню после генерации с фото дизайна)
# ➡️ СЛЕДУЮЩИЙ ЭКРАН: SCREEN 2 (загрузка фото, экран uploading_photo)
# 🔌 ТРИГГЕР: F.data == "uploading_photo" (when in post_generation state)
#
# 📋 ЛОГИКА:
# 1️⃣ Пользователь видит готовый дизайн + меню
# 2️⃣ Нажимает кнопку "📸 Новое фото"
# 3️⃣ УДАЛЯЕМ текущее меню (как и в change_style)
# 4️⃣ Переходим в состояние uploading_photo
# 5️⃣ Показываем экран загрузки фото
# 6️⃣ Пользователь загружает новое фото
# 7️⃣ Процесс начинается заново (SCREEN 3, выбор комнаты, выбор стиля, генерация)
#
# [2026-01-02 16:35] 🔥 NEW HANDLER: Реализует сценарий "Новое фото"
# - Похож на change_style но переходит на uploading_photo вместо choose_style_1
# - Должен ОЧИСТИТЬ FSM от previous_design данных (photo_id, selected_room, etc.)
# - Показать экран загрузки фото
#
# 📝 ЛОГИРОВАНИЕ:
# - "[V3] NEW_DESIGN+NEW_PHOTO - back to photo upload, user_id={user_id}"
...
async def new_photo_after_gen(callback: CallbackQuery, state: FSMContext):
    """
    📸 [SCREEN 6→2] new_photo_after_gen() - Загрузка нового фото после генерации
    
    📍 ПУТЬ: [SCREEN 6: дизайн готов] → нажать "📸 Новое фото" → [SCREEN 2: загрузка фото]
    ...
    [2026-01-02 16:35] 🔥 NEW HANDLER - КОПИРУЕТ СЦЕНАРИЙ CHANGE_STYLE
    
    📋 ЛОГИКА (ВАЖНО!):
    1️⃣ Юзер видит ФОТО дизайна + МЕНЮ с кнопками
    2️⃣ Нажимает "📸 Новое фото"
    3️⃣ РЕДАКТИРУЕМ ТОЛЬКО МЕНЮ (меняем содержимое на инструкции загрузки)
    4️⃣ ФОТО СТАРОГО дизайна остается для истории!
    5️⃣ Больше НИЧЕГО не генерируем!
    
    Затем при загрузке нового фото:
    → вызовется photo_handler() из creation_main.py
    → перейдет на room_choice (SCREEN 3)
    → процесс генерации начнется заново
    
    ✨ ЭФФЕКТ ДЛЯ ЮЗЕРА:
    "Я вижу свой дизайн, нажимаю 'новое фото', и вижу инструкцию загрузки.
     Загружаю новое фото и начинаю всё с начала (выбор комнаты, стиля)."
    
    🔄 ОТЛИЧИЕ ОТ CHANGE_STYLE:
    - change_style: показывает меню выбора СТИЛЕЙ
    - new_photo_after_gen: показывает меню загрузки ФОТО
    
    💾 ОЧИЩАЕТ:
    - НЕ очищаем всё состояние (state.clear())
    - Но переходим в новое состояние uploading_photo
    - work_mode сохраняется для удобства
    
    📤 ОТПРАВЛЯЕТ:
    - Редактированное МЕНЮ: "📄 Загрузите фото помещения"
    - Текст инструкций (из UPLOADING_PHOTO_TEMPLATES)
    - Пустая клавиатура (пользователь должен загрузить фото)
    
    💾 СОХРАНЯЕТ В БД:
    - screen_code = 'uploading_photo'
    
    ❌ НЕ ГЕНЕРИРУЕТ ДИЗАЙН!
    ❌ НЕ УДАЛЯЕТ ФОТО ДИЗАЙНА!
    ✅ РЕДАКТИРУЕТ ТОЛЬКО МЕНЮ!
    ✅ ПЕРЕХОДИТ В СОСТОЯНИЕ UPLOADING_PHOTO!
    ...
    logger.warning(f"🔍 [NEW_PHOTO] START: user_id={user_id}, menu_msg_id={menu_message_id}")

    data = await state.get_data()
    work_mode = data.get('work_mode', 'new_design')  # Сохраняем режим работы

    try:
        # ✅ РЕДАКТИРУЕМ ТЕКУЩЕЕ МЕНЮ НА ЗАГРУЗКУ ФОТО
        # Переходим в состояние загрузки фото
        await state.set_state(CreationStates.uploading_photo)
        
        # Формируем текст меню загрузки фото
        # Используем текст из UPLOADING_PHOTO_TEMPLATES
        text = UPLOADING_PHOTO_TEMPLATES.get(work_mode, "📄 Загрузите фото помещения")
        
        # 🔥 [2026-01-02 16:35] РЕДАКТИРУЕМ ТОЛЬКО МЕНЮ - БЕЗ ФОТО!
        await callback.bot.edit_message_text(
            chat_id=chat_id,
            message_id=menu_message_id,
            text=text,
            reply_markup=get_uploading_photo_keyboard(),  # Пустая клавиатура
            parse_mode="Markdown"
        )
        ...
        # ✅ Сохраняем новое состояние в FSM
        await state.update_data(
            menu_message_id=menu_message_id,
            photo_uploaded=False,  # Очищаем флаг загрузки (новое фото еще не загружено)
            new_photo=True  # Отмечаем что это новое фото
        )
        
        logger.info(f"✅ [NEW_PHOTO] Menu edited: msg_id={menu_message_id}, user_id={user_id}")
        logger.info(f"[V3] NEW_DESIGN+NEW_PHOTO - back to photo upload, user_id={user_id}")
        await callback.answer()
        
    except Exception as e:
        logger.error(f"[ERROR] NEW_PHOTO failed: {e}", exc_info=True)
        await callback.answer(
