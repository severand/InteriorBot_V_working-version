# bot/database/db.py
# --- ОБНОВЛЕН: 2026-01-02 21:41 - ФИНАЛЬНОЕ РЕШЕНИЕ: редактировать сообщение вместо удаления ---
...
class Database:
    ...
    async def get_or_create_menu_message(self, chat_id: int, user_id: int, bot, 
                                         text: str, reply_markup) -> Optional[int]:
        """
        🎯 [2026-01-02 21:41] ФИНАЛЬНОЕ РЕШЕНИЕ:
        
        Если меню ЕСТЬ в БД И FSM пусто → РЕДАКТИРОВАТЬ существующее
        Если меню НЕТ в БД → СОЗДАТЬ новое
        ...
        try:
            ...
            if menu_data and menu_data.get('menu_message_id'):
                ...
                try:
                    ...
                    logger.info(f"✏️ [EDIT_MENU] Отредактировано меню: chat={chat_id}, msg_id={old_menu_id}")
                    return old_menu_id
                except Exception as e:
                    logger.warning(f"⚠️ [EDIT_MENU] Не удалось отредактировать ({e}), создаем новое...")
                    # Если редактирование не удалось - создаем новое
                    pass
            
            # ✅ МЕНЮ НЕ СУЩЕСТВУЕТ → СОЗДАЕМ НОВОЕ
            ...
            # Сохраняем в БД
            await self.save_chat_menu(chat_id, user_id, new_msg.message_id)
            logger.info(f"📝 [CREATE_MENU] Создано новое меню: chat={chat_id}, msg_id={new_msg.message_id}")
            return new_msg.message_id
            
        except Exception as e:
            logger.error(f"❌ [GET_OR_CREATE_MENU] Ошибка: {e}")
            return None
