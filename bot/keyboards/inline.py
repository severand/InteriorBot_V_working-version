...
def get_uploading_photo_keyboard(has_previous_photo: bool = False) -> InlineKeyboardMarkup:
    """
    🆕 [2026-01-02 22:00] SCREEN 2: KEYBOARD С ДВУМЯ КНОПКАМИ (ДИНАМИЧЕСКИМИ)
    
    ОБНОВЛЕНО (2026-01-02):
    - Кнопка "📸 Использовать текущую фото" (ТОЛЬКО если есть в БД!)
    - Кнопка "🏠 Главное меню" (всегда)
    
    Параметры:
    - has_previous_photo: bool - проверяем в БД есть ли сохраненная фото
    
    Логика:
    Если has_previous_photo = False:
      - Показываем ТОЛЬКО кнопку "Главное меню"
    
    Если has_previous_photo = True:
      - Показываем ОБЫЧНО кнопку "Использовать текущую фото"
      - И кнопку "Главное меню" снизу
    
    Вызов из creation_main.py:
    1. [SCREEN 1→2] set_work_mode() → db.get_last_user_photo(user_id)
    2. Передает has_previous_photo в get_uploading_photo_keyboard()
    3. Клавиатура выстраивается динамически
    
    Обработчики:
    - "use_current_photo" → use_current_photo() handler
    - "select_mode" → перейти на SCREEN 1 выбора режимов
    """
    builder = InlineKeyboardBuilder()
