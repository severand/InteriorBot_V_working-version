...
# [2026-01-02 22:10] ✨ NEW: get_restart_menu_keyboard() - меню перезагрузки бота
#                    - Кнопка /start команды внизу справа
#                    - БЕЗ перезагрузки состояний и таблиц (bot stays in same position)
...
# ========================================
# 🔄 [2026-01-02 22:10] МЕНЮ ПЕРЕЗАГРУЗКИ БОТА
# БЕЗ ПОТЕРИ СОСТОЯНИЙ И ТАБЛИЦ
# ========================================

def get_restart_menu_keyboard() -> InlineKeyboardMarkup:
    """
    🔄 [2026-01-02 22:10] МЕНЮ ПЕРЕЗАГРУЗКИ БОТА
    
    НАЗНАЧЕНИЕ:
    - Добавляет кнопку меню внизу справа
    - Кнопка "🔄 Перезагрузить" с командой /start
    - БЕЗ перезагрузки FSM состояний
    - БЕЗ очистки таблиц базы данных
    - Бот остается в том же месте после перезагрузки
    
    ИСПОЛЬЗОВАНИЕ:
    - Может быть добавлена к ЛЮБОЙ существующей клавиатуре
    - Служит для мягкой перезагрузки бота
    - Сохраняет все данные юзера
    
    ЛОГИКА:
    1. Юзер нажимает "🔄 Перезагрузить"
    2. /start команда отправляется боту
    3. /start обрабатывает юзера на текущем экране
    4. Все состояния и данные сохраняются
    5. Бот показывает меню НА ТОМ ЖЕ экране
    
    РАСПОЛОЖЕНИЕ:
    - Одна кнопка внизу справа
    - Отдельный ряд в конце
    - Не конфликтует с другими кнопками
    
    ПРИМЕР ИСПОЛЬЗОВАНИЯ В ДРУГИХ ФУНКЦИЯХ:
    ```python
    def get_some_keyboard() -> InlineKeyboardMarkup:
        builder = InlineKeyboardBuilder()
        # ... существующие кнопки ...
        
        # Добавляем меню перезагрузки внизу
        restart_kb = get_restart_menu_keyboard()
        if restart_kb.inline_keyboard:
            builder.row(*restart_kb.inline_keyboard[0])
        
        return builder.as_markup()
    ```
    
    [2026-01-02 22:10] ✨ НОВАЯ ФУНКЦИЯ - ГОТОВА К ИСПОЛЬЗОВАНИЮ
    """
    builder = InlineKeyboardBuilder()
    
    # Одна кнопка внизу справа с командой /start
    builder.row(InlineKeyboardButton(
        text="🔄 Перезагрузить",
